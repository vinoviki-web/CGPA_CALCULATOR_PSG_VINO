...
num_semesters = st.number_input("How many semesters?", min_value=0, step=1)
gpa_list = []
credit_list = []
for i in range(int(num_semesters)):
    st.write(f"### Semester {i + 1}")
    gpa = st.number_input(f"Enter GPA for Semester {i + 1}:", min_value=0, max_value=10, step=0.01)
